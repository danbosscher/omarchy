...
class RoutingTest(unittest.TestCase):
  ...
  def apply(self):
    ...
    return [c.args[3] for c in self.resolver.call.call_args_list]

  def test_preserves_domains_route_and_enforced_tls(self):
    self.assertEqual(self.apply(), ['GetLink', 'SetLinkDNSEx'])
  ...
  def test_no_native_dns_default_uplink(self):
    ...
    self.assertEqual(self.apply(), ['GetLink', 'SetLinkDefaultRoute', 'SetLinkDNSEx'])
  ...
  def test_negative_priority_cannot_be_overridden(self):
    ...
    self.link['Domains'] = []
    self.assertEqual(self.apply(), ['GetLink'])
